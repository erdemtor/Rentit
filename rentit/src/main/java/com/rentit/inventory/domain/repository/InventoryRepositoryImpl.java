...
public class InventoryRepositoryImpl implements CustomInventoryRepository {
    ...
    public List<PlantInventoryEntry> findAvailable(String name, LocalDate startDate, LocalDate endDate) {
        TypedQuery<PlantInventoryEntry> query = em.createQuery(
                "select i.plantInfo from PlantInventoryItem i where lower(i.plantInfo.name) like ?1 and i.equipmentCondition = 'SERVICABLE' and i not in (select r.plant from PlantReservation r where ?2 < r.schedule.endDate and ?3 > r.schedule.startDate)"
